 ...
 *  заголовок панели), а цвет линии ничего не кодирует. Он один на все панели
 *  и живёт токеном `--data` в styles.css — намеренно вне набора состояний:
 *  зелёная линия читалась бы как «хорошая метрика», розовая — как «на неё
 *  можно нажать».
 */
      ...
      {data.length === 0 ? (
        ...
        <div className="chart-plot">
          <svg width={width || 300} height={HEIGHT} role="img" aria-label={title}>
            ...
            {data.length > 1 && <path className="chart-line" d={line} />}
