...
# Вывод сначала кладётся в переменную, и только потом проверяется. Через
# конвейер нельзя: `grep -q` завершается на первой же подошедшей строке,
# codesign получает SIGPIPE на второй, а `set -o pipefail` считает весь
# конвейер упавшим — проверка ругалась на совершенно исправную подпись.
VERIFY=$(codesign --verify --strict --verbose=2 "$APP" 2>&1 || true)
case "$VERIFY" in
  *"valid on disk"*) ;;
  *)
    echo >&2
